...
from SAC_discrete.networks import Critic, Actor, CriticLSTM, ActorLSTM
import copy
from params import *
...
class SAC(nn.Module):
    ...
    def __init__(self,
        ...
        if USE_LSTM:
            self.actor_local = ActorLSTM(state_size, action_size, hidden_size).to(device)
        else:
        ...
        if USE_LSTM:
            self.critic1 = CriticLSTM(state_size, action_size, hidden_size, 2).to(device)
            self.critic2 = CriticLSTM(state_size, action_size, hidden_size, 1).to(device)
        else:
        ...
        if USE_LSTM:
            self.critic1_target = CriticLSTM(state_size, action_size, hidden_size).to(device)
            self.critic1_target.load_state_dict(self.critic1.state_dict())

            self.critic2_target = CriticLSTM(state_size, action_size, hidden_size).to(device)
    ...
    def get_action_lstm(self, state, hidden):
        """Returns actions for given state as per current policy."""
        state = torch.from_numpy(state).float().to(self.device)
    ...
    def calc_policy_loss_lstm(self, states, alpha, hidden):
        _, action_probs, log_pis, hidden = self.actor_local.evaluate(states, hidden)

        q1, *_ = self.critic1(states, hidden)
        q2, *_ = self.critic2(states, hidden)
        min_Q = torch.min(q1, q2)
        actor_loss = (action_probs * (alpha * log_pis - min_Q)).sum(1).mean()
        log_action_pi = torch.sum(log_pis * action_probs, dim=1)
        return actor_loss, log_action_pi, hidden
    ...
    def learn(self, step, experiences, gamma, d=1):
        ...
        """
        assert USE_LSTM != True, 'Method supposed to work only with USE_LSTM=False. Check params.py'
    ...
    def learn_lstm(self, step, experiences, gamma, hidden):
        """Updates actor, critics and entropy_alpha parameters using given batch of experience tuples.
        Q_targets = r + γ * (min_critic_target(next_state, actor_target(next_state)) - α *log_pi(next_action|next_state))
        Critic_loss = MSE(Q, Q_target)
        Actor_loss = α * log_pi(a|s) - Q(s,a)
        ...
        # ---------------------------- update actor ---------------------------- #
        current_alpha = copy.deepcopy(self.alpha)
        actor_loss, log_pis, hidden = self.calc_policy_loss_lstm(states, current_alpha.to(self.device), hidden)
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()
        ...
        # Get predicted next-state actions and Q values from target models
        with torch.no_grad():
            _, action_probs, log_pis, hidden = self.actor_local.evaluate(next_states, hidden)
            Q_target1_next, hidden = self.critic1_target(next_states, hidden)
            Q_target2_next, hidden = self.critic2_target(next_states, hidden)
        ...
        return actor_loss.item(), alpha_loss.item(), critic1_loss.item(), critic2_loss.item(), current_alpha, hidden
